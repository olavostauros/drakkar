/**
 * Drakkar Theme - Unified JavaScript
 * Version 2.0 - Complete Unified Architecture
 *
 * All JavaScript functionality consolidated into a single file for:
...
(function () {
  ...
  const DrakkarTheme = {
    init: function () {
      ...
      if (DrakkarUtils.$('.hero-statistics') || DrakkarUtils.$('.stat-item')) {
        this.initStatisticsAnimation();
      }

      if (DrakkarUtils.$('.hero-video') || DrakkarUtils.$$('video').length) {
    ...
    setupSmoothScrolling: function () {
      ...
      // Hero CTA button specific smooth scroll
      const heroCtaButton = DrakkarUtils.$('.hero-cta-button');
      if (heroCtaButton) {
        heroCtaButton.addEventListener('click', (e) => {
          const targetElement = DrakkarUtils.$('#plataforma');
          if (targetElement) {
            e.preventDefault();
    ...
    // Statistics counter animation (unified from both files)
    ...
    // Counter animation logic (unified and enhanced)
    ...
    // Video optimization (unified and enhanced)
    ...
    initHeroFeatures: function () {
      // All hero-specific code is now integrated into the main functions above
      // This method exists for potential future hero-specific features
      // that don't fit into the general component systems

      // Example: Hero parallax effects, specific animations, etc.
      this.initHeroParallax();
    },

    // Hero parallax effects (placeholder for future enhancement)
    initHeroParallax: function () {
      const heroSection = DrakkarUtils.$('.hero-main, .hero-section');
      if (!heroSection) return;

      // Placeholder for parallax functionality if needed
      // Can be expanded based on design requirements
    }
  };
  ...
  // Ensure any external code expecting the old hero functions still works
  window.initStatisticsAnimation = function() {
    DrakkarTheme.initStatisticsAnimation();
  };

  window.initVideoOptimization = function() {
    DrakkarTheme.initVideoOptimization();
  };
